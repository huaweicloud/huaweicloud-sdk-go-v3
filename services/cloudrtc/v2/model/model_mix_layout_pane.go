...
type MixLayoutPane struct {
	...
	// 标识视频流的类型，可选摄像头流或者屏幕分享流。  默认为摄像头视频。

	VideoType *MixLayoutPaneVideoType `json:"video_type,omitempty"`
	// 坐标x，归一化百分比，画布上该画面左上角的横坐标的相对值，范围是 [0.0,1.0]。从左到右布局，0.0在最左端，1.0在最右端，小数取值范围在float32内，自定义布局场景下填写本字段。

	X float32 `json:"x,omitempty"`
	// 坐标y，归一化百分比，画布上该画面左上角的纵坐标的相对值，范围是 [0.0,1.0]。从上到下布局，0.0在最上端，1.0在最下端，小数取值范围在float32内，自定义布局场景下填写本字段。

	Y float32 `json:"y,omitempty"`
	// 窗格宽，归一化百分比，小数取值范围在float32内，自定义布局场景下填写本字段。
