...
type AbnormalEvent struct {
	...
	PeerUid string `json:"peer_uid" xml:"peer_uid"`

	// 异常类型： - 1：进房慢(5s加入房间失败) - 2：视频卡顿 - 3：音频卡顿
	AbnormalType int32 `json:"abnormal_type" xml:"abnormal_type"`

	// 异常类型描述与异常类型对应，支持国际化，取值范围为： - 进房慢(5s加入房间失败) - 视频卡顿 - 音频卡顿
	AbnormalTypeDesc string `json:"abnormal_type_desc" xml:"abnormal_type_desc"`
	...
	AbnormalFactor int32 `json:"abnormal_factor" xml:"abnormal_factor"`
	...
	AbnormalFactorDesc string `json:"abnormal_factor_desc" xml:"abnormal_factor_desc"`
}

func (o AbnormalEvent) String() string {
	data, err := utils.Marshal(o)
	if err != nil {
		return "AbnormalEvent struct{}"
	}

	return strings.Join([]string{"AbnormalEvent", string(data)}, " ")
}
