...
type ListInstancesByTagsRequestBody struct {
	...
	Action ListInstancesByTagsRequestBodyAction `json:"action" xml:"action"`

	// 搜索字段。   - 该字段值为空，表示不按照实例名称或实例ID查询。   - 该字段值不为空
	Matches *[]QueryMatchItem `json:"matches,omitempty" xml:"matches"`

	// 包含标签，最多包含10个key。
	Tags *[]QueryTagItem `json:"tags,omitempty" xml:"tags"`
}
