...
type SetAuditlogPolicyRequestBody struct {
	...
	ReserveAuditlogs *string `json:"reserve_auditlogs,omitempty" xml:"reserve_auditlogs"`

	// 仅打开审计日志策略时有效，并且为空时，默认全部。审计范围。请输入数据库或集合名称，多个库或集合请用英文逗号分隔。若名称中有英文逗号，请在逗号前添加“$”符号，用以区分分隔符。
	AuditScope *string `json:"audit_scope,omitempty" xml:"audit_scope"`

	// 仅打开审计日志策略时有效，并且为空时，默认全部。审计类型。支持insert，delete，update，query等。
	AuditTypes *[]string `json:"audit_types,omitempty" xml:"audit_types"`
}
