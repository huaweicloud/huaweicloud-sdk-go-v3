...
type PeriodReq struct {
	...
	// 订购周期类型。 - 2: 包月。 - 3: 包年。
	PeriodType int32 `json:"periodType" xml:"periodType"`

	// 订购周期数。 - 若选择包月（参数范围：1-9）。 - 若选择包年（参数范围：1-3）。
	PeriodNum int32 `json:"periodNum" xml:"periodNum"`

	// 是否自动续订，为空时表示不自动续订 - 1: 自动续订。 - 2: 不自动续订（默认）。
	IsAutoRenew *int32 `json:"isAutoRenew,omitempty" xml:"isAutoRenew"`
	...
	ConsoleURL *string `json:"consoleURL,omitempty" xml:"consoleURL"`
}
