...
type Setting struct {
	...
	Workers *int32 `json:"workers,omitempty" xml:"workers"`

	// 单个工作线程在尝试执行其Filters和Outputs之前将从inputs收集的最大事件数，该值较大通常更有效，但会增加内存开销，默认为125。
	BatchSize *int32 `json:"batchSize,omitempty" xml:"batchSize"`

	// 每个event被pipeline调度等待的最小时间。 单位毫秒。
	BatchDelayMs *int32 `json:"batchDelayMs,omitempty" xml:"batchDelayMs"`
	...
	QueueCheckPointWrites *int32 `json:"queueCheckPointWrites,omitempty" xml:"queueCheckPointWrites"`

	// 如果使用持久化队列，则表示持久化队列的总容量（以兆字节MB为单位），确保磁盘的容量大于该值，默认值为1024。
	QueueMaxBytesMb *int32 `json:"queueMaxBytesMb,omitempty" xml:"queueMaxBytesMb"`
}
