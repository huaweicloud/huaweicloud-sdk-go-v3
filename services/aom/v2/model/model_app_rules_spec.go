...
type AppRulesSpec struct {
	...
	AttrList *[]string `json:"attrList,omitempty" xml:"attrList"`

	// 是否开启日志采集。 true、false
	DetectLog string `json:"detectLog" xml:"detectLog"`
	...
	DiscoveryRule []DiscoveryRule `json:"discoveryRule" xml:"discoveryRule"`

	// 是否为默认规则。 true、false
	IsDefaultRule string `json:"isDefaultRule" xml:"isDefaultRule"`

	// 是否为规则预探测场景(预探测场景不会保存规则,仅用于规则下发之前验证该规则能否有效发现节点上的进程)。 true、false
	IsDetect string `json:"isDetect" xml:"isDetect"`

	// 日志文件的后缀。 log、trace、out
	LogFileFix []string `json:"logFileFix" xml:"logFileFix"`
	...
	LogPathRule *[]LogPathRule `json:"logPathRule,omitempty" xml:"logPathRule"`

	NameRule *NameRule `json:"nameRule" xml:"nameRule"`
