...
type CreateInstanceExtendParam struct {
	...
	ChargeMode *CreateInstanceExtendParamChargeMode `json:"charge_mode,omitempty" xml:"charge_mode"`

	// 订购周期类型，取值范围： - month：月。 - year：年。 “charge_mode”参数配置为“prePaid”时该参数有效且为必选值。
	PeriodType *CreateInstanceExtendParamPeriodType `json:"period_type,omitempty" xml:"period_type"`
	...
	IsAutoRenew *CreateInstanceExtendParamIsAutoRenew `json:"is_auto_renew,omitempty" xml:"is_auto_renew"`

	// 下单订购后，是否自动从客户的账户的余额中支付，取值范围： - “true”：是（自动从客户账户的余额中支付）。 - “false”：否（需要客户手动支付）。 “charge_mode”参数配置为“prePaid”时该参数有效，不传该字段时默认为客户手动支付。\"
	IsAutoPay *CreateInstanceExtendParamIsAutoPay `json:"is_auto_pay,omitempty" xml:"is_auto_pay"`
}
