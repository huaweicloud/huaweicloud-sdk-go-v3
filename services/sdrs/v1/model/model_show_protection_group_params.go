...
type ShowProtectionGroupParams struct {
	...
	// 该保护组中保护实例的个数。
	ProtectedInstanceNum int32 `json:"protected_instance_num" xml:"protected_instance_num"`

	// 该保护组中复制对的个数。
	ReplicationNum int32 `json:"replication_num" xml:"replication_num"`

	// 该保护组中容灾演练的个数。
	DisasterRecoveryDrillNum int32 `json:"disaster_recovery_drill_num" xml:"disaster_recovery_drill_num"`

	// 保护状态。started：表示该保护组开始保护。stopped：表示该保护组停止保护。 说明:系统近期进行了升级，对于升级后创建的保护组，该字段值为null，无实际意义。
	ProtectedStatus ShowProtectionGroupParamsProtectedStatus `json:"protected_status" xml:"protected_status"`
	...
	ReplicationStatus ShowProtectionGroupParamsReplicationStatus `json:"replication_status" xml:"replication_status"`

	// 健康状态。 normal：表示该保护组处于正常状态。 abnormal：表示该保护组处于非正常状态。  说明:系统近期进行了升级，对于升级后创建的保护组，该字段值为null，无实际意义。
	HealthStatus ShowProtectionGroupParamsHealthStatus `json:"health_status" xml:"health_status"`

	// 生产站点虚拟私有云ID。
	SourceVpcId string `json:"source_vpc_id" xml:"source_vpc_id"`

	// 容灾站点虚拟私有云ID。
	TargetVpcId string `json:"target_vpc_id" xml:"target_vpc_id"`

	// 容灾演练虚拟私有云ID。（该参数暂未使用）
	TestVpcId string `json:"test_vpc_id" xml:"test_vpc_id"`

	// 部署模式。默认值为“migration”，migration表示VPC内迁移。
	DrType string `json:"dr_type" xml:"dr_type"`
	...
	ProtectionType ShowProtectionGroupParamsProtectionType `json:"protection_type" xml:"protection_type"`

	// 复制类型。 说明:预留参数，暂未启用。
	ReplicationModel string `json:"replication_model" xml:"replication_model"`
