...
type BatchCreateProtectedInstancesRequestParams struct {
	...
	NamePrefix string `json:"name_prefix" xml:"name_prefix"`
	...
	PrimarySubnetId *string `json:"primary_subnet_id,omitempty" xml:"primary_subnet_id"`

	// 在专属主机或共享池中创建容灾站点云服务器，默认为在共享池中创建。值为：shared或dedicated。shared：表示共享池。dedicated：表示专属主机。
	Tenancy *BatchCreateProtectedInstancesRequestParamsTenancy `json:"tenancy,omitempty" xml:"tenancy"`
	...
	// 用于创建保护实例的云服务器信息列表。
	Servers []ServerInfo `json:"servers" xml:"servers"`
