...
type ShowReplicationParams struct {
	...
	// 复制对使用的云硬盘ID。
	VolumeIds string `json:"volume_ids" xml:"volume_ids"`

	// 挂载点。
	Attachment []ReplicationAttachment `json:"attachment" xml:"attachment"`
	...
	FailureDetail string `json:"failure_detail" xml:"failure_detail"`

	RecordMetadata *ReplicationRecordMetadata `json:"record_metadata" xml:"record_metadata"`

	// 复制对的故障等级。0：表示无故障。2：表示当前生产站点的云硬盘无读写数据权限，此时建议执行故障切换操作。5：表示复制链路已断，不能执行故障切换操作，需联系技术支持工程师。
	FaultLevel string `json:"fault_level" xml:"fault_level"`
	...
	ReplicationStatus ShowReplicationParamsReplicationStatus `json:"replication_status" xml:"replication_status"`
}
