...
type Subnet struct {
	...
	GatewayIp string `json:"gateway_ip" xml:"gateway_ip"`

	// 功能说明：是否创建cidr_v6 取值范围：true（开启），false（关闭）
	Ipv6Enable bool `json:"ipv6_enable" xml:"ipv6_enable"`

	// IPv6子网的网段，如果子网为IPv4子网，则不返回此参数
	CidrV6 string `json:"cidr_v6" xml:"cidr_v6"`

	// IPv6子网的网关，如果子网为IPv4子网，则不返回此参数
	GatewayIpV6 string `json:"gateway_ip_v6" xml:"gateway_ip_v6"`

	// 子网是否开启dhcp功能
	DhcpEnable bool `json:"dhcp_enable" xml:"dhcp_enable"`

	// 子网dns服务器地址1
	PrimaryDns string `json:"primary_dns" xml:"primary_dns"`

	// 子网dns服务器地址2
	SecondaryDns string `json:"secondary_dns" xml:"secondary_dns"`
	...
	Status SubnetStatus `json:"status" xml:"status"`

	// 对应网络（OpenStack Neutron接口）id
	NeutronNetworkId string `json:"neutron_network_id" xml:"neutron_network_id"`

	// 对应子网（OpenStack Neutron接口）id
	NeutronSubnetId string `json:"neutron_subnet_id" xml:"neutron_subnet_id"`

	// 对应IPv6子网（OpenStack Neutron接口）id，如果子网为IPv4子网，则不返回此参数
	NeutronSubnetIdV6 string `json:"neutron_subnet_id_v6" xml:"neutron_subnet_id_v6"`

	// 子网配置的NTP地址
	ExtraDhcpOpts []ExtraDhcpOption `json:"extra_dhcp_opts" xml:"extra_dhcp_opts"`
