...
type Port struct {
	...
	FixedIps []FixedIp `json:"fixed_ips" xml:"fixed_ips"`
	...
	DeviceOwner PortDeviceOwner `json:"device_owner" xml:"device_owner"`

	// 项目ID
	TenantId string `json:"tenant_id" xml:"tenant_id"`

	// 功能说明：端口状态，Hana硬直通虚拟机端口状态总为DOWN 取值范围：ACTIVE、BUILD、DOWN
	Status PortStatus `json:"status" xml:"status"`
	...
	AllowedAddressPairs []AllowedAddressPair `json:"allowed_address_pairs" xml:"allowed_address_pairs"`

	// 功能说明：DHCP的扩展Option(扩展属性)
	ExtraDhcpOpts []ExtraDhcpOpt `json:"extra_dhcp_opts" xml:"extra_dhcp_opts"`

	// 功能说明：绑定的vNIC类型 取值范围：  - normal（软交换）  - direct: SRIOV硬直通（不支持）
	BindingvnicType string `json:"binding:vnic_type" xml:"binding:vnic_type"`

	// 功能说明：主网卡默认内网域名信息 约束：不支持设置和更新，由系统自动维护
	DnsAssignment []DnsAssignMent `json:"dns_assignment" xml:"dns_assignment"`

	// 功能说明：主网卡默认内网DNS名称 约束：不支持设置和更新，由系统自动维护
	DnsName string `json:"dns_name" xml:"dns_name"`

	BindingvifDetails *BindingVifDetails `json:"binding:vif_details" xml:"binding:vif_details"`

	// 功能说明：提供用户设置自定义信息(扩展属性)
	Bindingprofile *interface{} `json:"binding:profile" xml:"binding:profile"`
	...
	PortSecurityEnabled bool `json:"port_security_enabled" xml:"port_security_enabled"`
