...
type SubNetworkInterface struct {
	...
	VirsubnetId string `json:"virsubnet_id" xml:"virsubnet_id"`

	// 功能说明：辅助弹性网卡的私有IPv4地址 取值范围：必须在虚拟子网的网段内，不填则随机在虚拟子网网段内随机分配
	PrivateIpAddress string `json:"private_ip_address" xml:"private_ip_address"`

	// 功能说明：辅助弹性网卡的IPv6地址
	Ipv6IpAddress string `json:"ipv6_ip_address" xml:"ipv6_ip_address"`

	// 功能说明：辅助弹性网卡的mac地址 取值范围：合法的mac地址，系统随机分配
	MacAddress string `json:"mac_address" xml:"mac_address"`

	// 功能说明：设备ID 取值范围：标准UUID
	ParentDeviceId string `json:"parent_device_id" xml:"parent_device_id"`
	...
	VlanId int32 `json:"vlan_id" xml:"vlan_id"`
