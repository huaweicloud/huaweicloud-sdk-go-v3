...
type BssParam struct {
	...
	IsAutoRenew *BssParamIsAutoRenew `json:"is_auto_renew,omitempty" xml:"is_auto_renew"`

	// 功能说明：付费方式（预付费、按需付费；预付费，即包周期付费）。 取值范围： - prePaid：预付费，即包年包月； - postPaid：后付费，即按需付费； 默认值是postPaid。 后付费的场景下，bss_param参数的其他字段都会被忽略。
	ChargingMode BssParamChargingMode `json:"charging_mode" xml:"charging_mode"`
	...
	IsAutoPay *BssParamIsAutoPay `json:"is_auto_pay,omitempty" xml:"is_auto_pay"`

	// 当选择包年包月时，该字段为必选，表示订购资源的周期类型。  取值范围如下： - month：表示包月 - year：表示包年
	PeriodType *BssParamPeriodType `json:"period_type,omitempty" xml:"period_type"`
