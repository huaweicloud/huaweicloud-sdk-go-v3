...
type CreateWebTamperProtectionConfigRequestBody struct {
	...
	ContainerWtpInfo *CreateWebTamperProtectionConfigRequestBodyContainerWtpInfo `json:"container_wtp_info,omitempty"`

	// **参数解释**: 是否立即开启防护 **约束限制**: 不涉及 **取值范围**: - true：立即开启防护 - false：只保存配置内容，暂不开启防护  **默认取值**: false
	OpenProtectionImmediately *bool `json:"open_protection_immediately,omitempty"`

	QuotaInfo *CreateWebTamperProtectionConfigRequestBodyQuotaInfo `json:"quota_info,omitempty"`
}

func (o CreateWebTamperProtectionConfigRequestBody) String() string {
	data, err := utils.Marshal(o)
	if err != nil {
		return "CreateWebTamperProtectionConfigRequestBody struct{}"
	}

	return strings.Join([]string{"CreateWebTamperProtectionConfigRequestBody", string(data)}, " ")
}
