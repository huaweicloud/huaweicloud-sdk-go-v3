...
type RestoreExistingInstanceRequestBodySource struct {
	...
	Type *RestoreExistingInstanceRequestBodySourceType `json:"type,omitempty" xml:"type"`

	// 用于恢复的备份ID。当使用备份文件恢复时需要指定该参数。
	BackupId *string `json:"backup_id,omitempty" xml:"backup_id"`

	// 恢复数据的时间点，格式为UNIX时间戳，单位是毫秒，时区为UTC。
	RestoreTime *int32 `json:"restore_time,omitempty" xml:"restore_time"`
	...
	DatabaseName map[string]string `json:"database_name,omitempty" xml:"database_name"`

	// 该字段仅适用于SQL Server引擎。是否恢复所有数据库，不填写该字段默认为false，不会恢复所有数据库到目标实例。 - 须知： 如果您想恢复所有数据库到已有实例，必须设置restore_all_database为true。
	RestoreAllDatabase *bool `json:"restore_all_database,omitempty" xml:"restore_all_database"`
}
