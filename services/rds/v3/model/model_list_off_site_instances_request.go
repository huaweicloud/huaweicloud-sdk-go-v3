...
type ListOffSiteInstancesRequest struct {
	...
	// 语言
	XLanguage *ListOffSiteInstancesRequestXLanguage `json:"X-Language,omitempty" xml:"X-Language"`

	// 索引位置，偏移量。从第一条数据偏移offset条数据后开始查询，默认为0（偏移0条数据，表示从第一条数据开始查询），必须为数字，不能为负数。
	Offset *interface{} `json:"offset,omitempty" xml:"offset"`

	// 查询记录数。默认为100，不能为负数，最小值为1，最大值为100。
	Limit *interface{} `json:"limit,omitempty" xml:"limit"`
}
