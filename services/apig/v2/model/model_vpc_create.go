...
type VpcCreate struct {
	...
	BalanceStrategy *VpcCreateBalanceStrategy `json:"balance_strategy,omitempty" xml:"balance_strategy"`

	// VPC通道的成员类型。 - ip - ecs  VPC通道类型为2时必选。
	MemberType *VpcCreateMemberType `json:"member_type,omitempty" xml:"member_type"`

	// VPC后端实例列表，VPC通道类型为1时，有且仅有1个后端实例。
	Members *[]MemberInfo `json:"members,omitempty" xml:"members"`

	VpcHealthConfig *VpcHealthConfig `json:"vpc_health_config,omitempty" xml:"vpc_health_config"`
}
