...
type ResettingAppSecretV2Response struct {
	...
	Creator *ResettingAppSecretV2ResponseCreator `json:"creator,omitempty" xml:"creator"`
	...
	// 状态   - 1： 有效
	Status *ResettingAppSecretV2ResponseStatus `json:"status,omitempty" xml:"status"`

	// APP的类型： - apig：存量apig应用，不推荐使用 - roma：roma集成应用  默认apig，暂不支持其他类型
	AppType *ResettingAppSecretV2ResponseAppType `json:"app_type,omitempty" xml:"app_type"`
