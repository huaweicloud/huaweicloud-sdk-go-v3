...
type ApiPolicyHttpResp struct {
	...
	EffectMode ApiPolicyHttpRespEffectMode `json:"effect_mode" xml:"effect_mode"`
	...
	// 策略条件列表
	Conditions []CoditionResp `json:"conditions" xml:"conditions"`
	...
	// 请求协议：HTTP、HTTPS
	ReqProtocol ApiPolicyHttpRespReqProtocol `json:"req_protocol" xml:"req_protocol"`

	// 请求方式：GET、POST、PUT、DELETE、HEAD、PATCH、OPTIONS、ANY
	ReqMethod ApiPolicyHttpRespReqMethod `json:"req_method" xml:"req_method"`
	...
	VpcChannelInfo *VpcInfo `json:"vpc_channel_info,omitempty" xml:"vpc_channel_info"`

	// 是否使用VPC通道： - 1： 使用VPC通道 - 2：不使用VPC通道
	VpcChannelStatus *int32 `json:"vpc_channel_status,omitempty" xml:"vpc_channel_status"`
}
