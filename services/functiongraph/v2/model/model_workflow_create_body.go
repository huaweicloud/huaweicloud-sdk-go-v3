...
type WorkflowCreateBody struct {
	...
	// 触发器列表
	Triggers *[]Trigger `json:"triggers,omitempty" xml:"triggers"`

	// 函数列表
	Functions []Function `json:"functions" xml:"functions"`

	// 函数流节点清单，定义参考SleepState和OperationState
	States []OperationState `json:"states" xml:"states"`

	// 函数流中的常量
	Constants *interface{} `json:"constants" xml:"constants"`

	// 重试策略清单
	Retries []Retry `json:"retries" xml:"retries"`
	...
	Mode *WorkflowCreateBodyMode `json:"mode,omitempty" xml:"mode"`

	ExpressConfig *ExpressConfig `json:"express_config,omitempty" xml:"express_config"`
