...
type ImportKeyMaterialRequestBody struct {
	...
	ImportToken string `json:"import_token" xml:"import_token"`

	// 加密后的对称密钥材料，base64格式，满足正则匹配“^[0-9a-zA-Z+/=]{344,360}$”。若导入非对称密钥，则该参数为用于加密私钥的临时中间密钥。
	EncryptedKeyMaterial string `json:"encrypted_key_material" xml:"encrypted_key_material"`

	// 使用临时中间密钥加密后的私钥，导入非对称密钥需要该参数，base64格式，满足正则匹配“^[0-9a-zA-Z+/=]{200,6144}$”。
	EncryptedPrivatekey *string `json:"encrypted_privatekey,omitempty" xml:"encrypted_privatekey"`
