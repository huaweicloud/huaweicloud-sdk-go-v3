...
type ListSecretStageInvoker struct {
	*invoker.BaseInvoker
}

func (i *ListSecretStageInvoker) Invoke() (*model.ListSecretStageResponse, error) {
	if result, err := i.BaseInvoker.Invoke(); err != nil {
		return nil, err
	} else {
		return result.(*model.ListSecretStageResponse), nil
	}
}
