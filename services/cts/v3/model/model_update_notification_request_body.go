...
type UpdateNotificationRequestBody struct {
	...
	// 标识关键操作名称。
	NotificationName string `json:"notification_name" xml:"notification_name"`

	// 标识操作类型。 目前支持的操作类型有完整类型(complete)和自定义类型(customized)。 完整类型下，CTS发送通知的对象为已对接服务的所有事件。 自定义类型下，CTS发送通知的对象是在operations列表中指定的事件。
	OperationType UpdateNotificationRequestBodyOperationType `json:"operation_type" xml:"operation_type"`
