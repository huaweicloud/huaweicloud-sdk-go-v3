...
type UpdateNotificationResponse struct {
	...
	OperationType *UpdateNotificationResponseOperationType `json:"operation_type,omitempty" xml:"operation_type"`

	// 操作事件列表。
	Operations *[]Operations `json:"operations,omitempty" xml:"operations"`

	// 通知用户列表，目前最多支持对10个用户组和50个用户发起的操作进行配置。
	NotifyUserList *[]NotificationUsers `json:"notify_user_list,omitempty" xml:"notify_user_list"`

	// 标识关键操作通知状态，包括正常(enabled)，停止(disabled)两种状态。
	Status *UpdateNotificationResponseStatus `json:"status,omitempty" xml:"status"`
	...
	// 关键操作通知的唯一标识。
	NotificationId *string `json:"notification_id,omitempty" xml:"notification_id"`

	// 关键操作通知类型，根据topic_id区分为消息通知服务(smn)和函数工作流(fun)。
	NotificationType *UpdateNotificationResponseNotificationType `json:"notification_type,omitempty" xml:"notification_type"`
