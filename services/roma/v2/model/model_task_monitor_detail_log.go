...
type TaskMonitorDetailLog struct {
	...
	Position *TaskMonitorDetailLogPosition `json:"position,omitempty" xml:"position"`

	// 任务当前步骤的状态，允许如下值：NORMAL-正在运行, NODE_END-本节点正常结束, RUNTIME_CANCEL-任务被取消, TASK_END-本任务正常结束, RUNTIME_ERR-运行时异常, INTERNAL_ERR-内部程序异常
	PositionStatus *TaskMonitorDetailLogPositionStatus `json:"position_status,omitempty" xml:"position_status"`

	// 标识当前步骤属于哪一个FDI插件，如adapter, apireader, rdbwriter等
	Stage *string `json:"stage,omitempty" xml:"stage"`
	...
	// 成功数据大小的计量单位
	DataSizeUnit *TaskMonitorDetailLogDataSizeUnit `json:"data_size_unit,omitempty" xml:"data_size_unit"`

	// 执行时长，单位：ms
	SpendTime *int32 `json:"spend_time,omitempty" xml:"spend_time"`

	// 执行详细信息
	Remarks *string `json:"remarks,omitempty" xml:"remarks"`

	// 本次步骤启动时间，格式timestamp(ms)，使用UTC时区
	StepBeginTime *int64 `json:"step_begin_time,omitempty" xml:"step_begin_time"`

	// 本次步骤结束时间，格式timestamp(ms)，使用UTC时区
	StepEndTime *int64 `json:"step_end_time,omitempty" xml:"step_end_time"`
}
