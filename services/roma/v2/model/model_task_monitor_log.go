...
type TaskMonitorLog struct {
	...
	DispatchTime *int64 `json:"dispatch_time,omitempty" xml:"dispatch_time"`
	...
	ExecuteStatus *TaskMonitorLogExecuteStatus `json:"execute_status,omitempty" xml:"execute_status"`

	// 标识本次任务执行到哪一个阶段，允许如下值：ADAPTER-任务处于初始化阶段, READER-任务正在执行Reader读操作, WRITER-任务正在执行Writer写操作
	Position *TaskMonitorLogPosition `json:"position,omitempty" xml:"position"`
	...
	PositionStatus *TaskMonitorLogPositionStatus `json:"position_status,omitempty" xml:"position_status"`
	...
	// 异常数据条数
	DirtyDataCount *int32 `json:"dirty_data_count,omitempty" xml:"dirty_data_count"`

	// 成功数据条数
	DataCount *int32 `json:"data_count,omitempty" xml:"data_count"`

	// 成功数据大小，浮点数类型
	DataSize float32 `json:"data_size,omitempty" xml:"data_size"`

	// 成功数据大小的计量单位
	DataSizeUnit *TaskMonitorLogDataSizeUnit `json:"data_size_unit,omitempty" xml:"data_size_unit"`

	// 执行时长，单位：ms
	SpendTime *int64 `json:"spend_time,omitempty" xml:"spend_time"`

	// 读取执行时长，单位：ms，只有在定时任务时存在该属性
	ReadSpendTime *int64 `json:"read_spend_time,omitempty" xml:"read_spend_time"`

	// 写入执行时长，单位：ms
	WriteSpendTime *int64 `json:"write_spend_time,omitempty" xml:"write_spend_time"`

	// 本次执行结果简要信息
	Remarks *string `json:"remarks,omitempty" xml:"remarks"`

	// 本次执行详细轨迹信息
	DetailLogs *[]TaskMonitorDetailLog `json:"detail_logs,omitempty" xml:"detail_logs"`
}
