...
type ApiConditionCreate struct {
	...
	SysParamName *ApiConditionCreateSysParamName `json:"sys_param_name,omitempty" xml:"sys_param_name"`

	// 策略条件 - exact：绝对匹配 - enum：枚举 - pattern：正则  策略类型为param时必选
	ConditionType *ApiConditionCreateConditionType `json:"condition_type,omitempty" xml:"condition_type"`

	// 策略类型 - param：参数 - source：源IP - system：系统参数
	ConditionOrigin ApiConditionCreateConditionOrigin `json:"condition_origin" xml:"condition_origin"`
