...
// ServiceInvokeCreateRequest **参数解释：**  服务调用时的相关配置。 **约束限制：**  不涉及。
type ServiceInvokeCreateRequest struct {
	...
	ElbConnection *ElbConnectionCreateRequest `json:"elb_connection,omitempty"`

	// **参数解释：** 服务的自定义调用地址。 **取值范围：**  不涉及。
	CustomUrls *[]string `json:"custom_urls,omitempty"`
}

func (o ServiceInvokeCreateRequest) String() string {
	data, err := utils.Marshal(o)
	if err != nil {
		return "ServiceInvokeCreateRequest struct{}"
	}

	return strings.Join([]string{"ServiceInvokeCreateRequest", string(data)}, " ")
}
