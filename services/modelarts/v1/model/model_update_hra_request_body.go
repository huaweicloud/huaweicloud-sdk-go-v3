...
// UpdateHraRequestBody HRA修改请求对象
type UpdateHraRequestBody struct {

	// **参数解释：** 工作空间ID。未创建工作空间时默认值为“0”，存在创建并使用的工作空间，以实际取值为准。 **取值范围：** 不涉及。
	WorkspaceId string `json:"workspace_id"`

	// **参数解释：** HRA规则列表。 **约束限制：** 不涉及。
	HraRules []HraRuleUpdateRequest `json:"hra_rules"`
...
func (o UpdateHraRequestBody) String() string {
	data, err := utils.Marshal(o)
	if err != nil {
		return "UpdateHraRequestBody struct{}"
	}

	return strings.Join([]string{"UpdateHraRequestBody", string(data)}, " ")
}
