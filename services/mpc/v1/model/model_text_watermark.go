...
type TextWatermark struct {
	...
	Referpos *string `json:"referpos,omitempty" xml:"referpos"`
	...
	// 字体，当前支持fzyouh
	FontName *string `json:"font_name,omitempty" xml:"font_name"`

	// 字体大小。  取值范围：[4, 120]
	FontSize *int32 `json:"font_size,omitempty" xml:"font_size"`

	// 字体颜色。 目前颜色支持 black，blue，white，green，red，yellow，brown，gold，pink，orange，purple。
	FontColor *string `json:"font_color,omitempty" xml:"font_color"`

	// 水印叠加母体 取值如下： - input：水印叠加在输入片源上，转码输出后实际大小按图像等比例缩放 - output：水印叠加在转码输出文件上。 默认值：input
	Base *TextWatermarkBase `json:"base,omitempty" xml:"base"`
}
