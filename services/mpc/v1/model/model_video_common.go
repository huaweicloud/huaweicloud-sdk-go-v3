...
type VideoCommon struct {
	...
	OutputPolicy *VideoCommonOutputPolicy `json:"output_policy,omitempty" xml:"output_policy"`

	// 视频编码格式。  取值如下： - 1：表示H.264。 - 2：表示H.265。
	Codec *int32 `json:"codec,omitempty" xml:"codec"`

	// 编码档次，建议设为3。  取值如下： - 1：VIDEO_PROFILE_H264_BASE - 2：VIDEO_PROFILE_H264_MAIN - 3：VIDEO_PROFILE_H264_HIGH - 4：VIDEO_PROFILE_H265_MAIN
	Profile *int32 `json:"profile,omitempty" xml:"profile"`
	...
	Preset *int32 `json:"preset,omitempty" xml:"preset"`

	// 最大参考帧数。  取值范围： - H264：[1，8]，默认值为4 。 - H265：固定值4。  单位：帧。
	RefFramesCount *int32 `json:"ref_frames_count,omitempty" xml:"ref_frames_count"`

	// I帧最大间隔  取值范围：[2，10]。  默认值：5。  单位：秒。
	MaxIframesInterval *int32 `json:"max_iframes_interval,omitempty" xml:"max_iframes_interval"`

	// 最大B帧间隔。  取值范围： - H264：[0，7]，默认值为4。 - H265：[0，7]，默认值为7。  单位：帧。
	BframesCount *int32 `json:"bframes_count,omitempty" xml:"bframes_count"`

	// 帧率  取值范围：0或[5,60]之间的整数，0表示自适应  单位：帧每秒
	FrameRate *int32 `json:"frame_rate,omitempty" xml:"frame_rate"`

	// 纵横比，图像缩放方式
	AspectRatio *int32 `json:"aspect_ratio,omitempty" xml:"aspect_ratio"`

	// 黑边剪裁类型  取值如下： - 0：不开启黑边剪裁 - 1：开启黑边剪裁，低复杂度算法，针对长视频（>5分钟） - 2：开启黑边剪裁，高复杂度算法，针对短视频（<=5分钟）
	BlackCut *int32 `json:"black_cut,omitempty" xml:"black_cut"`
}
