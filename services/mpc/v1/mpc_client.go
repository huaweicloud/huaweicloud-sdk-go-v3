...
// CreateEditingJobs 新建剪辑拼接任务
//
// 创建剪辑任务，用于将多个视频文件进行裁剪成多个视频分段，并且可以把这些视频分段合并成一个视频。
// 待剪辑的视频文件需要存储在与媒体处理服务同区域的OBS桶中，且该OBS桶已授权。
//
// Please refer to HUAWEI cloud API Explorer for details.
func (c *MpcClient) CreateEditingJobs(request *model.CreateEditingJobsRequest) (*model.CreateEditingJobsResponse, error) {
	requestDef := GenReqDefForCreateEditingJobs()

	if resp, err := c.HcClient.Sync(request, requestDef); err != nil {
		return nil, err
	} else {
		return resp.(*model.CreateEditingJobsResponse), nil
	}
}

// CreateEditingJobsInvoker 新建剪辑拼接任务
func (c *MpcClient) CreateEditingJobsInvoker(request *model.CreateEditingJobsRequest) *CreateEditingJobsInvoker {
	requestDef := GenReqDefForCreateEditingJobs()
	return &CreateEditingJobsInvoker{invoker.NewBaseInvoker(c.HcClient, request, requestDef)}
}

// DeleteEditingJobs 取消剪辑任务
//
// 取消已下发的生成剪辑任务，仅支持取消正在排队中的任务。
//
// Please refer to HUAWEI cloud API Explorer for details.
func (c *MpcClient) DeleteEditingJobs(request *model.DeleteEditingJobsRequest) (*model.DeleteEditingJobsResponse, error) {
	requestDef := GenReqDefForDeleteEditingJobs()

	if resp, err := c.HcClient.Sync(request, requestDef); err != nil {
		return nil, err
	} else {
		return resp.(*model.DeleteEditingJobsResponse), nil
	}
}

// DeleteEditingJobsInvoker 取消剪辑任务
func (c *MpcClient) DeleteEditingJobsInvoker(request *model.DeleteEditingJobsRequest) *DeleteEditingJobsInvoker {
	requestDef := GenReqDefForDeleteEditingJobs()
	return &DeleteEditingJobsInvoker{invoker.NewBaseInvoker(c.HcClient, request, requestDef)}
}

// ListEditingJobs 查询剪辑任务
//
// 查询剪辑任务的状态。
//
// Please refer to HUAWEI cloud API Explorer for details.
func (c *MpcClient) ListEditingJobs(request *model.ListEditingJobsRequest) (*model.ListEditingJobsResponse, error) {
	requestDef := GenReqDefForListEditingJobs()

	if resp, err := c.HcClient.Sync(request, requestDef); err != nil {
		return nil, err
	} else {
		return resp.(*model.ListEditingJobsResponse), nil
	}
}

// ListEditingJobsInvoker 查询剪辑任务
func (c *MpcClient) ListEditingJobsInvoker(request *model.ListEditingJobsRequest) *ListEditingJobsInvoker {
	requestDef := GenReqDefForListEditingJobs()
	return &ListEditingJobsInvoker{invoker.NewBaseInvoker(c.HcClient, request, requestDef)}
}
