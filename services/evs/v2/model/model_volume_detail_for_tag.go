...
type VolumeDetailForTag struct {
	...
	Attachments []Attachment `json:"attachments" xml:"attachments"`

	// 云硬盘所属的AZ信息。
	AvailabilityZone string `json:"availability_zone" xml:"availability_zone"`

	// 预留属性。
	OsVolHostAttrhost string `json:"os-vol-host-attr:host" xml:"os-vol-host-attr:host"`
	...
	OsVolTenantAttrtenantId string `json:"os-vol-tenant-attr:tenant_id" xml:"os-vol-tenant-attr:tenant_id"`

	// 云硬盘镜像的元数据。 > 说明： >  > 关于“volume_image_metadata”字段的详细说明，具体请参见：\"[查询镜像详情](https://support.huaweicloud.com/api-ims/ims_03_0703.html)\"。
	VolumeImageMetadata map[string]interface{} `json:"volume_image_metadata" xml:"volume_image_metadata"`

	// 云硬盘类型。 目前支持“SSD”，“SAS”和“SATA”三种。 “SSD”为超高IO云硬盘 “SAS”为高IO云硬盘 “SATA”为普通IO云硬盘
	VolumeType string `json:"volume_type" xml:"volume_type"`

	// 云硬盘大小，单位为GB。
	Size int32 `json:"size" xml:"size"`

	// 预留属性。
	ConsistencygroupId *string `json:"consistencygroup_id,omitempty" xml:"consistencygroup_id"`

	// 是否为启动云硬盘。 true：表示为启动云硬盘。 false：表示为非启动云硬盘。
	Bootable string `json:"bootable" xml:"bootable"`

	Metadata *VolumeMetadata `json:"metadata" xml:"metadata"`
	...
	// 预留属性。
	ReplicationStatus string `json:"replication_status" xml:"replication_status"`

	// 预留属性。
	OsVolumeReplicationextendedStatus string `json:"os-volume-replication:extended_status" xml:"os-volume-replication:extended_status"`

	// 预留属性。
	OsVolMigStatusAttrmigstat string `json:"os-vol-mig-status-attr:migstat" xml:"os-vol-mig-status-attr:migstat"`

	// 预留属性。
	OsVolMigStatusAttrnameId string `json:"os-vol-mig-status-attr:name_id" xml:"os-vol-mig-status-attr:name_id"`

	// 是否为共享云硬盘。true为共享盘，false为普通云硬盘。 该字段已经废弃，请使用multiattach。
	Shareable bool `json:"shareable" xml:"shareable"`
