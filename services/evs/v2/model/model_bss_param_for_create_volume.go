...
type BssParamForCreateVolume struct {
	...
	ChargingMode *BssParamForCreateVolumeChargingMode `json:"chargingMode,omitempty" xml:"chargingMode"`

	// 功能说明：是否立即支付。chargingMode为PrePaid时该参数会生效。默认值为false。 取值范围： * true：立即支付，从帐户余额中自动扣费 * false：不立即支付，创建订单暂不支付
	IsAutoPay *BssParamForCreateVolumeIsAutoPay `json:"isAutoPay,omitempty" xml:"isAutoPay"`

	// 功能说明：是否自动续订。chargingMode为prePaid时该参数会生效。默认值为false。 取值范围： * true：自动续订，自动续订周期与订购周期相同 * false：不自动续订
	IsAutoRenew *BssParamForCreateVolumeIsAutoRenew `json:"isAutoRenew,omitempty" xml:"isAutoRenew"`
	...
	PeriodType *BssParamForCreateVolumePeriodType `json:"periodType,omitempty" xml:"periodType"`
}
