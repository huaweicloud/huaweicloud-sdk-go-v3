...
type CreateAssetByFileUploadReq struct {
	...
	VideoName string `json:"video_name" xml:"video_name"`
	...
	VideoType string `json:"video_type" xml:"video_type"`
	...
	VideoMd5 *string `json:"video_md5,omitempty" xml:"video_md5"`

	// 封面图片文件类型。  取值如下： - JPG - PNG  上传后的封面名称是固定的，后缀名为封面类型缩写。例如cover0.jpg，cover1.png 若不指定类型，则封面文件无后缀名。  > 如果设置了图片格式，则不会执行首帧截图作为封面动作，需自行上传封面。
	CoverType *CreateAssetByFileUploadReqCoverType `json:"cover_type,omitempty" xml:"cover_type"`

	// 封面文件MD5值
	CoverMd5 *string `json:"cover_md5,omitempty" xml:"cover_md5"`

	// 字幕文件信息
	Subtitles *[]Subtitle `json:"subtitles,omitempty" xml:"subtitles"`

	// 视频标签。  单个标签不超过16个字节，最多不超过16个标签。  多个用逗号分隔，UTF8编码。
	Tags *string `json:"tags,omitempty" xml:"tags"`

	// 是否自动发布。  取值如下： - 0：表示不自动发布。 - 1：表示自动发布。  默认值：0。
	AutoPublish *CreateAssetByFileUploadReqAutoPublish `json:"auto_publish,omitempty" xml:"auto_publish"`
	...
	AutoPreheat *string `json:"auto_preheat,omitempty" xml:"auto_preheat"`
	...
	WorkflowName *string `json:"workflow_name,omitempty" xml:"workflow_name"`
}
