...
type WatermarkTemplate struct {
	...
	// 启用状态。  取值为： - 0：停用 - 1：启用
	Status *int64 `json:"status,omitempty" xml:"status"`
	...
	WatermarkType *string `json:"watermark_type,omitempty" xml:"watermark_type"`

	// type设置为Image时有效。  目前包括： - Original：只做简单缩放，不做其他处理 - Transparent：图片底色透明 - Grayed：彩色图片变灰
	ImageProcess *string `json:"image_process,omitempty" xml:"image_process"`
