...
type BandwidthResp struct {
	...
	ChargeMode *BandwidthRespChargeMode `json:"charge_mode,omitempty" xml:"charge_mode"`
	...
	ShareType *BandwidthRespShareType `json:"share_type,omitempty" xml:"share_type"`
	...
	Status *BandwidthRespStatus `json:"status,omitempty" xml:"status"`

	// 功能说明：是否开启企业级qos，仅共享带宽支持开启。（该字段仅在上海1局点返回）
	EnableBandwidthRules *bool `json:"enable_bandwidth_rules,omitempty" xml:"enable_bandwidth_rules"`

	// 功能说明：带宽支持的最大分组规则数。（该字段仅在上海1局点返回）
	RuleQuota *int32 `json:"rule_quota,omitempty" xml:"rule_quota"`

	// 功能说明：带宽规则对象（该字段仅在上海1局点返回）
	BandwidthRules *[]BandWidthRules `json:"bandwidth_rules,omitempty" xml:"bandwidth_rules"`
