...
type CreateDocWatermarkByAddressRequestBody struct {
	...
	// 待嵌入水印的文档类型。
	DocType CreateDocWatermarkByAddressRequestBodyDocType `json:"doc_type" xml:"doc_type"`
	...
	VisibleWatermark *string `json:"visible_watermark,omitempty" xml:"visible_watermark"`

	// 待嵌入的图形明水印文件的地址, 字段格式要求同src_file字段，图形文件的格式必须为“png”或“jpg”，否则返回参数错误；图像文件大小不超过1MB
	ImageMark *string `json:"image_mark,omitempty" xml:"image_mark"`
	...
	VisibleType *CreateDocWatermarkByAddressRequestBodyVisibleType `json:"visible_type,omitempty" xml:"visible_type"`
	...
	MarkedFilePassword *string `json:"marked_file_password,omitempty" xml:"marked_file_password"`

	// 添加水印后给文件设置只读密码， 最大支持长度256。默认不加只读密码。
	ReadonlyPassword *string `json:"readonly_password,omitempty" xml:"readonly_password"`

	// 明水印字体大小，取值为[1,100]，默认值50
	Front *int32 `json:"front,omitempty" xml:"front"`

	// 明水印旋转角度，逆时针方向，取值为[0,90]，默认值45。
	Rotation *int32 `json:"rotation,omitempty" xml:"rotation"`

	// 明水印的透明度，取值[0,1]，默认值为0.3；
	Opacity *float32 `json:"opacity,omitempty" xml:"opacity"`
}
