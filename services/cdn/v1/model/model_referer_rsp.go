...
type RefererRsp struct {
	...
	RefererType *int32 `json:"referer_type,omitempty" xml:"referer_type"`

	// 请输入域名或IP地址，以“;”进行分割，域名、IP地址可以混合输入，支持泛域名添加。输入的域名、IP地址总数不超过100个。当设置防盗链时，此项必填。
	RefererList *string `json:"referer_list,omitempty" xml:"referer_list"`

	// 是否包含空Referer。如果是黑名单并开启该选项，则表示无referer不允许访问。如果是白名单并开启该选项，则表示无referer允许访问。默认不包含。
	IncludeEmpty *bool `json:"include_empty,omitempty" xml:"include_empty"`
}
