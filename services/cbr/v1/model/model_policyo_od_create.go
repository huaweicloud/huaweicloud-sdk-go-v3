...
type PolicyoOdCreate struct {
	...
	DayBackups *int32 `json:"day_backups,omitempty" xml:"day_backups"`
	...
	MaxBackups *int32 `json:"max_backups,omitempty" xml:"max_backups"`

	// 保留月备个数，该备份不受保留最大备份数限制。取值为0到100。若选择该参数，则timezone 也必选。
	MonthBackups *int32 `json:"month_backups,omitempty" xml:"month_backups"`

	// 备份保留时长，单位天。最长支持99999天。-1代表不按时间清理。若该字段和max_backups 参数同时为空，备份会永久保留。
	RetentionDurationDays *int32 `json:"retention_duration_days,omitempty" xml:"retention_duration_days"`
	...
	WeekBackups *int32 `json:"week_backups,omitempty" xml:"week_backups"`

	// 保留年备个数，该备份不受保留最大备份数限制。取值为0到100。若选择该参数，则timezone 也必选。
	YearBackups *int32 `json:"year_backups,omitempty" xml:"year_backups"`
}
