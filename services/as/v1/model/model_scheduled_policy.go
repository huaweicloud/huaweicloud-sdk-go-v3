...
type ScheduledPolicy struct {
	...
	LaunchTime string `json:"launch_time" xml:"launch_time"`

	// 周期触发类型，scaling_policy_type为RECURRENCE时该项必选。Daily：每天执行一次。Weekly：每周指定天执行一次。Monthly：每月指定天执行一次。
	RecurrenceType *ScheduledPolicyRecurrenceType `json:"recurrence_type,omitempty" xml:"recurrence_type"`
	...
	RecurrenceValue *string `json:"recurrence_value,omitempty" xml:"recurrence_value"`
