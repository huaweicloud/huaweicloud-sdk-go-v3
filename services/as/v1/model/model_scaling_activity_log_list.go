...
type ScalingActivityLogList struct {
	...
	Status *ScalingActivityLogListStatus `json:"status,omitempty" xml:"status"`
	...
	InstanceRemovedList *string `json:"instance_removed_list,omitempty" xml:"instance_removed_list"`

	// 完成伸缩活动且被移出弹性伸缩组并删除的云服务器名称列表，云服务器名之间以逗号分隔。
	InstanceDeletedList *string `json:"instance_deleted_list,omitempty" xml:"instance_deleted_list"`

	// 完成伸缩活动且被加入弹性伸缩组的云服务器名称列表，云服务器名之间以逗号分割。
	InstanceAddedList *string `json:"instance_added_list,omitempty" xml:"instance_added_list"`
