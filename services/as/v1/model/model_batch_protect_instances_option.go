...
type BatchProtectInstancesOption struct {
	...
	InstanceDelete *BatchProtectInstancesOptionInstanceDelete `json:"instance_delete,omitempty" xml:"instance_delete"`

	// 批量操作实例action标识：添加：ADD  移除： REMOVE  设置实例保护： PROTECT  取消实例保护： UNPROTECT；转入备用状态：ENTER_STANDBY 移出备用状态:EXIT_STANDBY
	Action BatchProtectInstancesOptionAction `json:"action" xml:"action"`

	// 将实例移入备用状态时，是否补充新的云服务器。取值如下：no：不补充新的实例，默认情况为no。yes：补充新的实例。只有action为ENTER_STANDBY时，这个字段才生效。
	InstanceAppend *BatchProtectInstancesOptionInstanceAppend `json:"instance_append,omitempty" xml:"instance_append"`
}
