...
type CreateScalingPolicyV2Option struct {
	...
	ScalingPolicyName string `json:"scaling_policy_name" xml:"scaling_policy_name"`
	...
	ScalingResourceId string `json:"scaling_resource_id" xml:"scaling_resource_id"`

	// 伸缩资源类型。伸缩组：SCALING_GROUP。带宽：BANDWIDTH。
	ScalingResourceType CreateScalingPolicyV2OptionScalingResourceType `json:"scaling_resource_type" xml:"scaling_resource_type"`

	// 策略类型。告警策略：ALARM（与alarm_id对应）；定时策略：SCHEDULED（与scheduled_policy对应）；周期策略：RECURRENCE（与scheduled_policy对应）
	ScalingPolicyType CreateScalingPolicyV2OptionScalingPolicyType `json:"scaling_policy_type" xml:"scaling_policy_type"`
