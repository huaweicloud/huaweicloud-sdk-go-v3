...
type UpdateScalingV2PolicyOption struct {
	...
	ScalingResourceType *UpdateScalingV2PolicyOptionScalingResourceType `json:"scaling_resource_type,omitempty" xml:"scaling_resource_type"`

	// 策略类型。告警策略：ALARM（与alarm_id对应）；定时策略：SCHEDULED（与scheduled_policy对应）；周期策略：RECURRENCE（与scheduled_policy对应）
	ScalingPolicyType *UpdateScalingV2PolicyOptionScalingPolicyType `json:"scaling_policy_type,omitempty" xml:"scaling_policy_type"`
