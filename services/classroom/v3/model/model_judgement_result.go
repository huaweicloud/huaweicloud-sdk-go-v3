...
type JudgementResult struct {
	...
	// 用例形式输出的用例总个数
	CaseCount int32 `json:"case_count" xml:"case_count"`

	// 用例形式输出的已执行用例的个数
	ExecutedCount int32 `json:"executed_count" xml:"executed_count"`

	// 用例形式输出的已执行用例的结果
	Testcases []JudgementCaseResult `json:"testcases" xml:"testcases"`
}
