...
type ListImageByTagsRequestBody struct {
	...
	Action ListImageByTagsRequestBodyAction `json:"action" xml:"action"`
	...
	TagsAny *[]Tags `json:"tags_any,omitempty" xml:"tags_any"`

	// 不包含标签，最多包含10个key，每个key对应的values最多包含10个值，且key和values都不能重复。不能为空列表。
	NotTags *[]Tags `json:"not_tags,omitempty" xml:"not_tags"`

	// 不包含任意标签，最多包含10个key，每个key对应的values最多包含10个值，且key和values都不能重复。不能为空列表。
	NotTagsAny *[]Tags `json:"not_tags_any,omitempty" xml:"not_tags_any"`
	...
	Matches *[]TagKeyValue `json:"matches,omitempty" xml:"matches"`
