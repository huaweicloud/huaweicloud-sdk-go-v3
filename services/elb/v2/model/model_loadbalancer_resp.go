...
type LoadbalancerResp struct {
	...
	VipSubnetId string `json:"vip_subnet_id" xml:"vip_subnet_id"`
	...
	// 负载均衡器关联的监听器ID的列表
	Listeners []ResourceList `json:"listeners" xml:"listeners"`

	// 负载均衡器关联的后端云服务器组ID的列表。
	Pools []ResourceList `json:"pools" xml:"pools"`

	// 负载均衡器的供应者名称。只支持vlb
	Provider string `json:"provider" xml:"provider"`

	// 负载均衡器的操作状态
	OperatingStatus LoadbalancerRespOperatingStatus `json:"operating_status" xml:"operating_status"`

	// 负载均衡器的配置状态
	ProvisioningStatus LoadbalancerRespProvisioningStatus `json:"provisioning_status" xml:"provisioning_status"`
