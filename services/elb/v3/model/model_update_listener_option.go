...
type UpdateListenerOption struct {
	...
	InsertHeaders *ListenerInsertHeaders `json:"insert_headers,omitempty" xml:"insert_headers"`

	// 监听器的名称。
	Name *string `json:"name,omitempty" xml:"name"`

	// 监听器使用的SNI证书（带域名的服务器证书）ID列表。  使用说明： - 列表对应的所有SNI证书的域名不允许存在重复。 - 列表对应的所有SNI证书的域名总数不超过30。
	SniContainerRefs *[]string `json:"sni_container_refs,omitempty" xml:"sni_container_refs"`

	// 监听器使用的SNI证书泛域名匹配方式。 longest_suffix表示最长尾缀匹配，wildcard表示标准域名分级匹配。 默认为wildcard。
	SniMatchAlgo *string `json:"sni_match_algo,omitempty" xml:"sni_match_algo"`
	...
	SecurityPolicyId *string `json:"security_policy_id,omitempty" xml:"security_policy_id"`
	...
	EnableMemberRetry *bool `json:"enable_member_retry,omitempty" xml:"enable_member_retry"`

	// 等待后端服务器响应超时时间。请求转发后端服务器后，在等待超时member_timeout时长没有响应，负载均衡将终止等待，并返回 HTTP504错误码。   取值：1-300s。   使用说明：仅支持协议为HTTP/HTTPS的监听器。
	MemberTimeout *int32 `json:"member_timeout,omitempty" xml:"member_timeout"`

	// 等待客户端请求超时时间，仅限协议为HTTP，HTTPS的监听器配置。取值范围为1-300s, 默认值为60s TCP，UDP协议的监听器不支持此字段
	ClientTimeout *int32 `json:"client_timeout,omitempty" xml:"client_timeout"`
	...
	KeepaliveTimeout *int32 `json:"keepalive_timeout,omitempty" xml:"keepalive_timeout"`

	Ipgroup *UpdateListenerIpGroupOption `json:"ipgroup,omitempty" xml:"ipgroup"`
	...
	EnhanceL7policyEnable *bool `json:"enhance_l7policy_enable,omitempty" xml:"enhance_l7policy_enable"`

	QuicConfig *UpdateListenerQuicConfigOption `json:"quic_config,omitempty" xml:"quic_config"`
}
