...
// PortProtocol **参数解释**： 设置Kafka实例的接入方式。PLAINTEXT表示明文接入，SASL_SSL或者SASL_PLAINTEXT表示密文接入。 **约束限制**： 内网访问不支持关闭，明文接入和密文接入至少开启一个。 跨VPC访问的安全协议等于内网访问的安全协议，若内网同时开启了密文访问和明文访问，则跨VPC访问的安全协议会优先使用密文访问的安全协议。
