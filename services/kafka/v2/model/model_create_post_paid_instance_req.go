...
type CreatePostPaidInstanceReq struct {
	...
	// 消息引擎。取值填写为：kafka。
	Engine CreatePostPaidInstanceReqEngine `json:"engine" xml:"engine"`

	// 消息引擎的版本。取值填写为：1.1.0和2.3.0。
	EngineVersion CreatePostPaidInstanceReqEngineVersion `json:"engine_version" xml:"engine_version"`
	...
	Specification *CreatePostPaidInstanceReqSpecification `json:"specification,omitempty" xml:"specification"`
	...
	PartitionNum *CreatePostPaidInstanceReqPartitionNum `json:"partition_num,omitempty" xml:"partition_num"`
	...
	KafkaManagerUser string `json:"kafka_manager_user" xml:"kafka_manager_user"`

	// 表示登录Kafka Manager的密码。  复杂度要求：   - 输入长度为8到32位的字符串。   - 必须包含如下四种字符中的两种组合：       - 小写字母       - 大写字母       - 数字       - 特殊字符包括（`~!@#$%^&*()-_=+\\|[{}]:'\",<.>/?）
	KafkaManagerPassword string `json:"kafka_manager_password" xml:"kafka_manager_password"`
	...
	RetentionPolicy *CreatePostPaidInstanceReqRetentionPolicy `json:"retention_policy,omitempty" xml:"retention_policy"`
	...
	StorageSpecCode CreatePostPaidInstanceReqStorageSpecCode `json:"storage_spec_code" xml:"storage_spec_code"`
