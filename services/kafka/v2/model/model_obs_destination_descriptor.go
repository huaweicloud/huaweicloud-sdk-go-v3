...
type ObsDestinationDescriptor struct {
	...
	TopicsRegex *string `json:"topics_regex,omitempty" xml:"topics_regex"`

	// 转储启动偏移量：   - latest: 从Topic最后端开始消费。   - earliest: 从Topic最前端消息开始消费。  默认是latest。
	ConsumerStrategy ObsDestinationDescriptorConsumerStrategy `json:"consumer_strategy" xml:"consumer_strategy"`

	// 转储文件格式。当前只支持text。
	DestinationFileType ObsDestinationDescriptorDestinationFileType `json:"destination_file_type" xml:"destination_file_type"`
	...
	ObsPath *string `json:"obs_path,omitempty" xml:"obs_path"`
	...
	PartitionFormat *string `json:"partition_format,omitempty" xml:"partition_format"`

	// 转储文件的记录分隔符，用于分隔写入转储文件的用户数据。 取值范围：   - 逗号“,”   - 分号“;”   - 竖线“|”   - 换行符“\\n”   - NULL  默认值：换行符“\\n”。
	RecordDelimiter *string `json:"record_delimiter,omitempty" xml:"record_delimiter"`

	// 根据用户配置的时间，周期性的将数据导入OBS，若某个时间段内无数据，则此时间段不会生成打包文件。 取值范围：30～900 单位：秒。 > 使用OBS通道转储流式数据时该参数为必选配置。
	DeliverTimeInterval int32 `json:"deliver_time_interval" xml:"deliver_time_interval"`
}
