...
type NodeContentCompareOverviewResult struct {
	...
	// 已完成表数。
	CompareEndNum *int32 `json:"compare_end_num,omitempty"`

	// 不一致表数。
	DataInconsistentNum *int32 `json:"data_inconsistent_num,omitempty"`

	// 无法对比表数。
	UncomparableNum *int32 `json:"uncomparable_num,omitempty"`
}

func (o NodeContentCompareOverviewResult) String() string {
	data, err := utils.Marshal(o)
	if err != nil {
		return "NodeContentCompareOverviewResult struct{}"
	}

	return strings.Join([]string{"NodeContentCompareOverviewResult", string(data)}, " ")
}
