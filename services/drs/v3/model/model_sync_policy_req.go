...
type SyncPolicyReq struct {
	...
	ConflictPolicy *SyncPolicyReqConflictPolicy `json:"conflict_policy,omitempty" xml:"conflict_policy"`

	// 过滤DDL策略。
	FilterDdlPolicy *SyncPolicyReqFilterDdlPolicy `json:"filter_ddl_policy,omitempty" xml:"filter_ddl_policy"`

	// 同步增量是否同步DDL。
	DdlTrans *bool `json:"ddl_trans,omitempty" xml:"ddl_trans"`
	...
	TopicPolicy *SyncPolicyReqTopicPolicy `json:"topic_policy,omitempty" xml:"topic_policy"`
	...
	PartitionPolicy *SyncPolicyReqPartitionPolicy `json:"partition_policy,omitempty" xml:"partition_policy"`

	// 投送到kafka的数据格式，不填默认为json：
	KafkaDataFormat *SyncPolicyReqKafkaDataFormat `json:"kafka_data_format,omitempty" xml:"kafka_data_format"`
	...
	TopicNameFormat *string `json:"topic_name_format,omitempty" xml:"topic_name_format"`

	// Partition个数，取值1-2147483647之间，topic_policy为1,2,3,时需要，不填默认为1
	PartitionsNum *string `json:"partitions_num,omitempty" xml:"partitions_num"`

	// 副本个数，取值1-32767之间，topic_policy为1,2,3,时需要，不填默认为1
	ReplicationFactor *string `json:"replication_factor,omitempty" xml:"replication_factor"`

	// PostgreSQL同步全量阶段是否填充物化视图，不填默认为false
	IsFillMaterializedView *bool `json:"is_fill_materialized_view,omitempty" xml:"is_fill_materialized_view"`

	// PostgreSQL同步全量阶段是否使用快照模式导出，不填默认为false
	ExportSnapshot *bool `json:"export_snapshot,omitempty" xml:"export_snapshot"`

	// 复制槽名称，gaussdbv5ha-to-kafka主备任务必填
	SlotName *string `json:"slot_name,omitempty" xml:"slot_name"`
}
