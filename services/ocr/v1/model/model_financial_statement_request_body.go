...
type FinancialStatementRequestBody struct {
	...
	ReturnTextLocation *bool `json:"return_text_location,omitempty" xml:"return_text_location"`

	// 返回字段识别置信度，小数点后四位。可选值包括：  - true：返回字段置信度;  - false：不返回。  > 说明：  - 未传入该参数时默认为false，即不返回字段置信度。
	ReturnConfidence *bool `json:"return_confidence,omitempty" xml:"return_confidence"`
	...
	ReturnExcel *bool `json:"return_excel,omitempty" xml:"return_excel"`

	// 返回表格坐标，可选值包括：  - true：返回表格坐标;  - false：不返回。  > 说明：  - 未传入该参数时默认为false，即不返回。
	ReturnTableLocation *bool `json:"return_table_location,omitempty" xml:"return_table_location"`

	// 返回矫正后的图像大小，可选值包括：  - true：返回矫正图像大小;  - false：不返回。  > 说明：  - 未传入该参数时默认为false，即不返回。
	ReturnImageSize *bool `json:"return_image_size,omitempty" xml:"return_image_size"`
}
