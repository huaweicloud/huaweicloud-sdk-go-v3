...
type OpenMysqlProxyRequestBody struct {
	...
	ProxyName *string `json:"proxy_name,omitempty" xml:"proxy_name"`

	// 代理实例类型。默认类型为readwrite。
	ProxyMode *OpenMysqlProxyRequestBodyProxyMode `json:"proxy_mode,omitempty" xml:"proxy_mode"`

	// 数据库节点的读权重设置。
	NodesReadWeight *[]NodesWeight `json:"nodes_read_weight,omitempty" xml:"nodes_read_weight"`
}
