...
type TaskNodeGroups struct {
	...
	DataVolumeType TaskNodeGroupsDataVolumeType `json:"data_volume_type" xml:"data_volume_type"`

	// Task节点数据磁盘存储数目，取值范围：0～10。
	DataVolumeCount int32 `json:"data_volume_count" xml:"data_volume_count"`

	// Task节点数据磁盘存储大小。  取值范围：100GB～32000GB，传值只需填数字，不需要带单位GB。
	DataVolumeSize int32 `json:"data_volume_size" xml:"data_volume_size"`
