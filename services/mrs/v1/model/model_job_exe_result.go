...
type JobExeResult struct {
	...
	// 执行程序jar包或sql文件地址。
	JarPath string `json:"jar_path" xml:"jar_path"`
	...
	// 作业日志存储地址
	JobLog string `json:"job_log" xml:"job_log"`
	...
	// 导入导出数据。
	FileAction string `json:"file_action" xml:"file_action"`

	// 程序执行的关键参数，该参数由用户程序内的函数指定，MRS只负责参数的传入。该参数可为空。
	Arguments string `json:"arguments" xml:"arguments"`

	// HQL脚本语句。
	Hql string `json:"hql" xml:"hql"`

	// 作业状态编码：  - -1：Terminated表示已终止的作业状态 - 2：Running表示运行中的作业状态 - 3：Completed表示已完成的作业状态 - 4：Abnormal表示异常的作业状态
	JobState int32 `json:"job_state" xml:"job_state"`

	// 作业最终状态码。  - 0：未完成 - 1：执行错误，终止执行 - 2：执行完成并且成功 - 3：已取消
	JobFinalStatus int32 `json:"job_final_status" xml:"job_final_status"`

	// Hive脚本地址。
	HiveScriptPath string `json:"hive_script_path" xml:"hive_script_path"`

	// 创建作业的用户ID。
	CreateBy string `json:"create_by" xml:"create_by"`

	// 当前已完成的步骤数。
	FinishedStep int32 `json:"finished_step" xml:"finished_step"`

	// 作业主ID。
	JobMainId string `json:"job_main_id" xml:"job_main_id"`

	// 作业步骤ID。
	JobStepId string `json:"job_step_id" xml:"job_step_id"`

	// 延迟时间，十三位时间戳。
	PostponeAt int64 `json:"postpone_at" xml:"postpone_at"`

	// 作业步骤名。
	StepName string `json:"step_name" xml:"step_name"`

	// 步骤数量。
	StepNum int32 `json:"step_num" xml:"step_num"`

	// 任务数量。
	TaskNum int32 `json:"task_num" xml:"task_num"`

	// 更新作业的用户ID。
	UpdateBy string `json:"update_by" xml:"update_by"`

	// 作业执行持续时间，单位：秒。
	SpendTime float32 `json:"spend_time" xml:"spend_time"`

	// 步骤序列号。
	StepSeq int32 `json:"step_seq" xml:"step_seq"`
