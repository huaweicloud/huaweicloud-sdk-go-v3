...
type CreateClusterReq struct {
	...
	ClusterVersion string `json:"cluster_version" xml:"cluster_version"`
	...
	MasterNodeNum int32 `json:"master_node_num" xml:"master_node_num"`

	// Core节点数量。 取值范围：[1～500] Core节点默认的最大值为500，如果用户需要的Core节点数大于500，请申请扩大配额。
	CoreNodeNum int32 `json:"core_node_num" xml:"core_node_num"`

	// 集群的计费模式。  12：表示按需计费。接口调用仅支持创建按需计费集群。
	BillingType CreateClusterReqBillingType `json:"billing_type" xml:"billing_type"`

	// 集群区域信息，请参见[终端节点及区域](https://support.huaweicloud.com/api-mrs/mrs_02_0003.html)。
	DataCenter string `json:"data_center" xml:"data_center"`
	...
	MasterNodeSize string `json:"master_node_size" xml:"master_node_size"`
	...
	CoreNodeSize string `json:"core_node_size" xml:"core_node_size"`

	// 服务组件安装列表信息。
	ComponentList []ComponentList `json:"component_list" xml:"component_list"`
	...
	AvailableZoneId string `json:"available_zone_id" xml:"available_zone_id"`
	...
	SecurityGroupsId *string `json:"security_groups_id,omitempty" xml:"security_groups_id"`

	// 创建集群时可同时提交作业，当前版本暂时只支持新增一个作业。
	AddJobs *[]AddJobs `json:"add_jobs,omitempty" xml:"add_jobs"`
	...
	VolumeType *CreateClusterReqVolumeType `json:"volume_type,omitempty" xml:"volume_type"`

	// 该参数为多磁盘参数，表示Master节点数据磁盘存储类别，目前支持SATA、SAS、SSD和GPSSD。
	MasterDataVolumeType *CreateClusterReqMasterDataVolumeType `json:"master_data_volume_type,omitempty" xml:"master_data_volume_type"`

	// 该参数为多磁盘参数，表示Master节点数据磁盘存储空间。为增大数据存储容量，创建集群时可同时添加磁盘。  取值范围：100GB～32000GB，传值只需填数字，不需要带单位GB。
	MasterDataVolumeSize *int32 `json:"master_data_volume_size,omitempty" xml:"master_data_volume_size"`

	// 该参数为多磁盘参数，表示Master节点数据磁盘个数。取值只能是1。
	MasterDataVolumeCount *CreateClusterReqMasterDataVolumeCount `json:"master_data_volume_count,omitempty" xml:"master_data_volume_count"`

	// 该参数为多磁盘参数，表示Core节点数据磁盘存储类别，目前支持SATA、SAS、SSD和GPSSD。
	CoreDataVolumeType *CreateClusterReqCoreDataVolumeType `json:"core_data_volume_type,omitempty" xml:"core_data_volume_type"`

	// 该参数为多磁盘参数，表示Core节点数据磁盘存储空间。为增大数据存储容量，创建集群时可同时添加磁盘。  取值范围：100GB～32000GB，传值只需填数字，不需要带单位GB。
	CoreDataVolumeSize *int32 `json:"core_data_volume_size,omitempty" xml:"core_data_volume_size"`

	// 该参数为多磁盘参数，表示Core节点数据磁盘个数。 取值范围：1～10
	CoreDataVolumeCount *int32 `json:"core_data_volume_count,omitempty" xml:"core_data_volume_count"`

	// Task节点列表信息。
	TaskNodeGroups *[]TaskNodeGroups `json:"task_node_groups,omitempty" xml:"task_node_groups"`

	// 配置引导操作脚本信息。
	BootstrapScripts *[]BootstrapScript `json:"bootstrap_scripts,omitempty" xml:"bootstrap_scripts"`

	// 密钥对名称。用户可以使用密钥对方式登录集群节点。当“login_mode”配置为“1”时，请求消息体中包含node_public_cert_name字段。
	NodePublicCertName *string `json:"node_public_cert_name,omitempty" xml:"node_public_cert_name"`

	// 配置MRS Manager管理员用户的密码。 - 密码长度应在8～26个字符之间 - 不能与用户名或者倒序用户名相同 - 必须包含如下4种字符的组合     - 至少一个小写字母     - 至少一个大写字母     - 至少一个数字     - 至少一个特殊字符：!@$%^-_=+[{}]:,./?
	ClusterAdminSecret *string `json:"cluster_admin_secret,omitempty" xml:"cluster_admin_secret"`
	...
	ClusterMasterSecret string `json:"cluster_master_secret" xml:"cluster_master_secret"`
	...
	SafeMode CreateClusterReqSafeMode `json:"safe_mode" xml:"safe_mode"`

	// 集群类型。  默认值为0：分析集群。  说明：暂不支持通过接口方式创建混合集群。  枚举值： - 0：分析集群 - 1：流式集群
	ClusterType *CreateClusterReqClusterType `json:"cluster_type,omitempty" xml:"cluster_type"`

	// 集群创建失败时，是否收集失败日志。  默认设置为1，将创建OBS桶仅用于MRS集群创建失败时的日志收集。  枚举值： - 0：不收集 - 1：收集
	LogCollection *CreateClusterReqLogCollection `json:"log_collection,omitempty" xml:"log_collection"`
	...
	LoginMode *CreateClusterReqLoginMode `json:"login_mode,omitempty" xml:"login_mode"`
	...
	NodeGroups *[]NodeGroupV11 `json:"node_groups,omitempty" xml:"node_groups"`
}
