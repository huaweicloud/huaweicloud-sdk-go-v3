...
type CreateClusterReqV2 struct {
	...
	IsDecProject *bool `json:"is_dec_project,omitempty" xml:"is_dec_project"`
	...
	ChargeInfo *ChargeInfo `json:"charge_info,omitempty" xml:"charge_info"`
	...
	AutoCreateDefaultSecurityGroup *bool `json:"auto_create_default_security_group,omitempty" xml:"auto_create_default_security_group"`
	...
	SafeMode string `json:"safe_mode" xml:"safe_mode"`

	// 配置Manager管理员用户的密码。 - 密码长度应在8～26个字符之间。 - 至少包含四种字符组合，如大写字母，小写字母，数字，特殊字符（!@$%^-_=+[{}]:,./?），但不能包含空格。 - 不能与用户名或者倒序用户名相同。
	ManagerAdminPassword string `json:"manager_admin_password" xml:"manager_admin_password"`

	// 节点登录方式。 - PASSWORD：密码登录，选择此项时，node_root_password不能为空。 - KEYPAIR：密钥对登录，选择此项时，node_keypair_name不能为空。
	LoginMode string `json:"login_mode" xml:"login_mode"`

	// 配置访问集群节点的root密码。 密码设置约束如下： - 字符串类型，可输入的字符串长度为8-26。 - 至少包含四种字符组合，如大写字母，小写字母，数字，特殊字符（!@$%^-_=+[{}]:,./?），但不能包含空格。 - 不能与用户名或者倒序用户名相同。
	NodeRootPassword *string `json:"node_root_password,omitempty" xml:"node_root_password"`

	// 密钥对名称。用户可以使用密钥对方式登录集群节点。
	NodeKeypairName *string `json:"node_keypair_name,omitempty" xml:"node_keypair_name"`
	...
	MrsEcsDefaultAgency *string `json:"mrs_ecs_default_agency,omitempty" xml:"mrs_ecs_default_agency"`
	...
	LogCollection *CreateClusterReqV2LogCollection `json:"log_collection,omitempty" xml:"log_collection"`

	// 组成集群的节点组信息。
	NodeGroups []NodeGroupV2 `json:"node_groups" xml:"node_groups"`
