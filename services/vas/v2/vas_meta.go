...
import (
	...
	"github.com/RandolphCYG/huaweicloud-sdk-go-v3/services/vas/v2/model"
	"net/http"
)
...
func GenReqDefForStartTask() *def.HttpRequestDef {
	reqDefBuilder := def.NewHttpRequestDefBuilder().
		WithMethod(http.MethodPost).
		WithPath("/v2/{project_id}/services/{service_name}/tasks/{task_id}/start").
...
func GenReqDefForStopTask() *def.HttpRequestDef {
	reqDefBuilder := def.NewHttpRequestDefBuilder().
		WithMethod(http.MethodPost).
		WithPath("/v2/{project_id}/services/{service_name}/tasks/{task_id}/stop").
