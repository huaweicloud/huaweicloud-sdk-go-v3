...
type CreateServers struct {
	...
	Metadata *MetaDataInfo `json:"metadata" xml:"metadata"`
	...
	SecurityGroups *[]SecurityGroupsInfo `json:"security_groups,omitempty" xml:"security_groups"`
	...
	RootVolume *RootVolume `json:"root_volume,omitempty" xml:"root_volume"`

	// 裸金属服务器对应数据盘相关配置。每一个数据结构代表一个待创建的数据盘。详情请参见表 data_volumes字段数据结构说明。约束：目前裸金属服务器最多可挂载60块云硬盘（包括系统盘和数据盘）。
	DataVolumes *[]DataVolumes `json:"data_volumes,omitempty" xml:"data_volumes"`

	Extendparam *ExtendParam `json:"extendparam" xml:"extendparam"`

	SchedulerHints *CreateSchedulerHints `json:"schedulerHints,omitempty" xml:"schedulerHints"`

	// 裸金属服务器的标签。详情请参见表 server_tags字段数据结构说明。 说明：创建裸金属服务器时，一台裸金属服务器最多可以添加10个标签。其中，__type_baremetal为系统内部标签，因此实际能添加的标签为9个。
	ServerTags *[]SystemTags `json:"server_tags,omitempty" xml:"server_tags"`
}
