...
type MetadataLock struct {
	...
	LockStatus string `json:"lock_status" xml:"lock_status"`
	...
	LockDuration string `json:"lock_duration" xml:"lock_duration"`

	// 锁所在的数据库，对于部分Global read lock级别的元数据锁，该值为空。
	TableSchema string `json:"table_schema" xml:"table_schema"`
	...
	// 事务执行时间
	TrxExecTime string `json:"trx_exec_time" xml:"trx_exec_time"`

	// 阻塞会话列表
	BlockProcess []Process `json:"block_process" xml:"block_process"`

	// 等待会话列表
	WaitProcess []Process `json:"wait_process" xml:"wait_process"`
}
