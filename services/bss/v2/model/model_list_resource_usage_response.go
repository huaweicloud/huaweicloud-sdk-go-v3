...
type ListResourceUsageResponse struct {
	...
	ActualDays *string `json:"actual_days,omitempty" xml:"actual_days"`

	// 计费带宽的按月汇总。 说明： 每月2日20点后可查询上月数据；若查询当月数据，则为空。
	BandWidth *string `json:"band_width,omitempty" xml:"band_width"`

	// 月保底带宽的按月汇总。 说明： 每月2日20点后可查询上月数据；若查询当月数据，则为空。该字段为预留值，当前始终为空；当场景为95增强时才返回数值。
	MonthlyGuaranteedBandWidth *string `json:"monthly_guaranteed_band_width,omitempty" xml:"monthly_guaranteed_band_width"`

	// 月峰值带宽。 说明： 每月2日20点后可查询上月数据；若查询当月数据，则为空。该字段为预留值，当前始终为空；当场景为95增强时才返回数值。
	MonthlyPeakBandWidth *string `json:"monthly_peak_band_width,omitempty" xml:"monthly_peak_band_width"`

	// 带宽单位，您可以调用查询度量单位列表接口获取。若所有带宽为空，则该字段为空。
	BandWidthMeasureId *int32 `json:"band_width_measure_id,omitempty" xml:"band_width_measure_id"`

	// 总条数。
	TotalCount *int32 `json:"total_count,omitempty" xml:"total_count"`

	// 使用量明细（5分钟统计值）。具体请参见表3。
	UsageInfoList  *[]StatUsageInfo `json:"usage_info_list,omitempty" xml:"usage_info_list"`
