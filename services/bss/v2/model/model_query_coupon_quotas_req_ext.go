...
type QueryCouponQuotasReqExt struct {
	...
	QuotaIds *[]string `json:"quota_ids,omitempty" xml:"quota_ids"`

	// 优惠券额度状态列表。 0：正常3：失效（过期失效和人工设置失效）4：额度调整中（伙伴可以查看该额度，但不能使用该额度发放优惠券）5：冻结 此参数不携带或携带值为空列表或携带值为null时，不作为筛选条件。
	QuotaStatusList *[]int32 `json:"quota_status_list,omitempty" xml:"quota_status_list"`
	...
	EffectiveTimeBegin *string `json:"effective_time_begin,omitempty" xml:"effective_time_begin"`
	...
	EffectiveTimeEnd *string `json:"effective_time_end,omitempty" xml:"effective_time_end"`
	...
	ExpireTimeBegin *string `json:"expire_time_begin,omitempty" xml:"expire_time_begin"`
	...
	ExpireTimeEnd *string `json:"expire_time_end,omitempty" xml:"expire_time_end"`
