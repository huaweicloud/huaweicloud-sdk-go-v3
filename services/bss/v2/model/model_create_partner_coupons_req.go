...
type CreatePartnerCouponsReq struct {
	...
	CustomerIds []string `json:"customer_ids" xml:"customer_ids"`

	// 代金券面值。 单位：元。取值大于0且精确到小数点后2位。
	FaceValue float64 `json:"face_value" xml:"face_value"`
	...
	CloudServiceTypes *[]string `json:"cloud_service_types,omitempty" xml:"cloud_service_types"`

	// 允许使用的产品列表。 多个产品以“,”隔开，最多支持10个。 默认：空（继承额度上的限制）  说明： 如果额度上有限制产品列表，则优惠券上的限制不能超过额度的限制。如果额度上没有限制，则优惠券上可以随意指定产品ID。 产品ID需要合作伙伴通过线下获得。
	ProductIds *[]string `json:"product_ids,omitempty" xml:"product_ids"`
