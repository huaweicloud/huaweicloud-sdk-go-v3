...
type ListEnterpriseSubCustomersRequest struct {
	...
	SubCustomerAccountName *string `json:"sub_customer_account_name,omitempty" xml:"sub_customer_account_name"`

	// 企业子账号的显示名称。不限制特殊字符。根据fuzzy_query取值决定是否按模糊查询。此参数不携带或携带值为空时，不作为筛选条件；携带值为空串时，作为筛选条件。
	SubCustomerDisplayName *string `json:"sub_customer_display_name,omitempty" xml:"sub_customer_display_name"`

	// 企业子账号的显示名称、用户名是否按模糊查询。默认值为“0：不按模糊查询”。0：不按模糊查询1：按模糊查询
	FuzzyQuery *int32 `json:"fuzzy_query,omitempty" xml:"fuzzy_query"`
	...
	OrgId *string `json:"org_id,omitempty" xml:"org_id"`
}
