...
type CreateSubCustomerReqV2 struct {
	...
	PartyId string `json:"party_id" xml:"party_id"`

	// 企业子账号的显示名称，不限制特殊字符。
	DisplayName *string `json:"display_name,omitempty" xml:"display_name"`

	// 子账号关联类型：1：同一法人。 关联类型目前只能是同一法人。
	SubCustomerAssociationType *int32 `json:"sub_customer_association_type,omitempty" xml:"sub_customer_association_type"`

	// 申请的权限列表。 支持的权限项请参见下表。
	PermissionIds *[]string `json:"permission_ids,omitempty" xml:"permission_ids"`

	NewSubCustomer *NewCustomerV2 `json:"new_sub_customer" xml:"new_sub_customer"`
}
