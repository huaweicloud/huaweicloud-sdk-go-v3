...
type ChangeEnterpriseRealnameAuthsReq struct {
	...
	// 认证方案： 1：单位证件扫描
	IdentifyType int32 `json:"identify_type" xml:"identify_type"`
	...
	VerifiedFileUrl []string `json:"verified_file_url" xml:"verified_file_url"`

	// 单位名称。 不能全是数字、特殊字符、空格。
	CorpName string `json:"corp_name" xml:"corp_name"`

	// 单位证件号码。
	VerifiedNumber string `json:"verified_number" xml:"verified_number"`

	// 实名认证填写的注册国家。国家的两位字母简码。 例如：注册国家为“中国”请填写“CN”。 此参数不携带或携带值为null时，不赋值；携带值为空串时，赋值空串。
	RegCountry *string `json:"reg_country,omitempty" xml:"reg_country"`

	// 实名认证企业注册地址。 此参数不携带或携带值为null时，不赋值；携带值为空串时，赋值空串。
	RegAddress *string `json:"reg_address,omitempty" xml:"reg_address"`

	// 变更类型： 1：个人变企业
	ChangeType int32 `json:"change_type" xml:"change_type"`

	// 华为分给合作伙伴的平台标识。 该标识的具体值由华为分配。获取方法请参见如何获取xaccountType的取值。
	XaccountType string `json:"xaccount_type" xml:"xaccount_type"`

	EnterprisePerson *EnterprisePersonNew `json:"enterprise_person,omitempty" xml:"enterprise_person"`
}
