...
type ListEndpointServiceRequest struct {
	...
	Status *ListEndpointServiceRequestStatus `json:"status,omitempty" xml:"status"`

	// 查询结果中终端节点服务列表的排序字段，取值为： ● create_at：终端节点服务的创建时间 ● update_at：终端节点服务的更新时间 默认值为create_at。
	SortKey *ListEndpointServiceRequestSortKey `json:"sort_key,omitempty" xml:"sort_key"`

	// 查询结果中终端节点服务列表的排 序方式，取值为： ● desc：降序排序 ● asc：升序排序 默认值为desc。
	SortDir *ListEndpointServiceRequestSortDir `json:"sort_dir,omitempty" xml:"sort_dir"`
