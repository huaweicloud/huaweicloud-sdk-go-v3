...
type CreateEndpointServiceResponse struct {
	...
	ServerType *CreateEndpointServiceResponseServerType `json:"server_type,omitempty" xml:"server_type"`
	...
	ApprovalEnabled *bool `json:"approval_enabled,omitempty" xml:"approval_enabled"`

	// 终端节点服务的状态。 ● creating：创建中 ● available：可连接 ● failed：失败
	Status *CreateEndpointServiceResponseStatus `json:"status,omitempty" xml:"status"`
	...
	CidrType *CreateEndpointServiceResponseCidrType `json:"cidr_type,omitempty" xml:"cidr_type"`

	// 服务开放的端口映射列表，详细内容请参见表 4-13 同一个终端节点服务下，不允许重复的端口映 射。若多个终端节点服务共用一个port_id，则 终端节点服务之间的所有端口映射的 server_port和protocol的组合不能重复。
	Ports *[]PortList `json:"ports,omitempty" xml:"ports"`
	...
	TcpProxy *CreateEndpointServiceResponseTcpProxy `json:"tcp_proxy,omitempty" xml:"tcp_proxy"`
