...
type UpdateEndpointWhiteResponse struct {
	...
	ServiceType *UpdateEndpointWhiteResponseServiceType `json:"service_type,omitempty" xml:"service_type"`

	// 终端节点的连接状态。 ● pendingAcceptance：待接受 ● creating：创建中 ● accepted：已接受 ● failed：失败
	Status *UpdateEndpointWhiteResponseStatus `json:"status,omitempty" xml:"status"`
