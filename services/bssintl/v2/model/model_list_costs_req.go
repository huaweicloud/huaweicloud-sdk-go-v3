...
type ListCostsReq struct {
	TimeCondition *TimeCondition `json:"time_condition" xml:"time_condition"`

	// 查询维度，具体请参见表 GroupBy。
	Groupby []GroupBy `json:"groupby" xml:"groupby"`

	// 成本类型。UNBLENDED_COST：原始成本AMORTIZED_COST：摊销成本
	CostType string `json:"cost_type" xml:"cost_type"`

	// 展示的金额类型。PAYMENT_AMOUNT：应付NET_AMOUNT：实付
	AmountType string `json:"amount_type" xml:"amount_type"`
	...
	Filters *[]FilterV2 `json:"filters,omitempty" xml:"filters"`
}
