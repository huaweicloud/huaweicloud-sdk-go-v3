...
type PrePaidServerDataVolume struct {
	...
	Volumetype PrePaidServerDataVolumeVolumetype `json:"volumetype" xml:"volumetype"`
	...
	Extendparam *PrePaidServerDataVolumeExtendParam `json:"extendparam,omitempty" xml:"extendparam"`

	// 云服务器数据盘对应的磁盘存储类型。 磁盘存储类型枚举值： DSS：专属存储类型
	ClusterType *PrePaidServerDataVolumeClusterType `json:"cluster_type,omitempty" xml:"cluster_type"`

	// 数据镜像的ID，UUID格式。
	ClusterId *string `json:"cluster_id,omitempty" xml:"cluster_id"`

	Metadata *PrePaidServerDataVolumeMetadata `json:"metadata,omitempty" xml:"metadata"`
