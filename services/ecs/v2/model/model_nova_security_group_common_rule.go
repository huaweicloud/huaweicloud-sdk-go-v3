...
type NovaSecurityGroupCommonRule struct {
	...
	FromPort int32 `json:"from_port" xml:"from_port"`

	Group *NovaSecurityGroupCommonGroup `json:"group" xml:"group"`

	// 安全组规则ID，UUID格式。
	Id string `json:"id" xml:"id"`

	// 协议类型或直接指定IP协议号，取值可为icmp，tcp，udp或IP协议号。
	IpProtocol string `json:"ip_protocol" xml:"ip_protocol"`

	IpRange *NovaSecurityGroupCommonIpRange `json:"ip_range" xml:"ip_range"`

	// 相关联的安全组ID，UUID格式。
	ParentGroupId string `json:"parent_group_id" xml:"parent_group_id"`

	// 终止端口，范围1-65535，且不小于from_port。 ip_protocol设置为icmp时，to_port表示code，范围是0-255，且如果from_port为-1，to_port为-1表示任意ICMP报文。
	ToPort int32 `json:"to_port" xml:"to_port"`
}
