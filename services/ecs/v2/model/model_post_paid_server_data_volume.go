...
type PostPaidServerDataVolume struct {
	...
	Volumetype PostPaidServerDataVolumeVolumetype `json:"volumetype" xml:"volumetype"`

	// 数据盘大小，容量单位为GB，输入大小范围为[10,32768]。
	Size int32 `json:"size" xml:"size"`

	// 是否为共享磁盘。true为共享盘，false为普通云硬盘。  > 说明： >  > 该字段已废弃，请使用multiattach。
	Shareable *bool `json:"shareable,omitempty" xml:"shareable"`
	...
	Hwpassthrough *bool `json:"hw:passthrough,omitempty" xml:"hw:passthrough"`

	Extendparam *PostPaidServerDataVolumeExtendParam `json:"extendparam,omitempty" xml:"extendparam"`

	// 云服务器数据盘对应的磁盘存储类型。 磁盘存储类型枚举值： DSS：专属存储类型
	ClusterType *PostPaidServerDataVolumeClusterType `json:"cluster_type,omitempty" xml:"cluster_type"`

	// 云服务器数据盘对应的存储池的ID。
	ClusterId *string `json:"cluster_id,omitempty" xml:"cluster_id"`

	Metadata *PostPaidServerDataVolumeMetadata `json:"metadata,omitempty" xml:"metadata"`

	// 数据镜像的ID，UUID格式。  如果使用数据盘镜像创建数据盘，则data_image_id为必选参数，且不支持使用metadata。
	DataImageId *string `json:"data_image_id,omitempty" xml:"data_image_id"`
}
