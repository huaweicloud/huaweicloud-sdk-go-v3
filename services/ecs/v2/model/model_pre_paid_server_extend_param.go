...
type PrePaidServerExtendParam struct {
	...
	ChargingMode *PrePaidServerExtendParamChargingMode `json:"chargingMode,omitempty" xml:"chargingMode"`
	...
	PeriodType *PrePaidServerExtendParamPeriodType `json:"periodType,omitempty" xml:"periodType"`
	...
	IsAutoRenew *PrePaidServerExtendParamIsAutoRenew `json:"isAutoRenew,omitempty" xml:"isAutoRenew"`

	// 下单订购后，是否自动从客户的账户中支付，而不需要客户手动去进行支付。  - “true”：是（自动支付） - “false”：否（需要客户手动支付）  > 说明： >  > chargingMode为prePaid时生效，不传该字段时默认为客户手动支付。
	IsAutoPay *PrePaidServerExtendParamIsAutoPay `json:"isAutoPay,omitempty" xml:"isAutoPay"`
	...
	SupportAutoRecovery *bool `json:"support_auto_recovery,omitempty" xml:"support_auto_recovery"`

	// 创建竞价实例时，需指定该参数的值为“spot”。  > 说明： >  > 当chargingMode=postPaid且marketType=spot时，此参数生效。
	MarketType *string `json:"marketType,omitempty" xml:"marketType"`
	...
	SpotPrice *string `json:"spotPrice,omitempty" xml:"spotPrice"`

	// 是否支持先创建卷，再创建虚拟机。  “true”：配置该功能 “false”：不配置该功能
	DiskPrior *string `json:"diskPrior,omitempty" xml:"diskPrior"`
	...
	SpotDurationHours *int32 `json:"spot_duration_hours,omitempty" xml:"spot_duration_hours"`

	// 竞价实例中断策略，当前支持immediate。  - 当interruption_policy=immediate时表示释放策略为立即释放。
	InterruptionPolicy *PrePaidServerExtendParamInterruptionPolicy `json:"interruption_policy,omitempty" xml:"interruption_policy"`
	...
	SpotDurationCount *int32 `json:"spot_duration_count,omitempty" xml:"spot_duration_count"`
}
